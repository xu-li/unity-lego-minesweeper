...
public class CheckMineTrigger : SensoryTrigger
{
    public enum Type
    {
        Up,
    ...
    [SerializeField,
     Tooltip("Trigger on input pressed.\nor\nOn input released.\nor\nWhile input pressed.\nor\nWhile input released.")]
    ...
    [SerializeField] GameObject m_InputPromptPrefab = default;
    ...
    protected override void Start()
    {
        ...
        if (IsPlacedOnBrick())
        {
            if (m_Enable != Enable.Always)
            {
                ...
                var sensoryCollider = LEGOBehaviourCollider.Add<SensoryCollider>(colliderComponentToClone,
                    m_ScopedBricks, m_Distance * LEGOHorizontalModule);
    ...
    bool CheckAxis(string axisName, Direction axisDirection)
    {
        ...
        var axisAbsoluteValue = axisDirection == Direction.Positive
            ? Mathf.Max(0f, axisRaw)
            : Mathf.Abs(Mathf.Min(axisRaw, 0f));
